...
static void log_level_struct_tt__VideoEncoderConfiguration(unsigned int                          level,
                                                           struct tt__VideoEncoderConfiguration* vecfg)
{
    if (NULL == vecfg) {
        return;
    }
...
static void log_level_struct_tt__SystemCapabilities(unsigned int level, struct tt__SystemCapabilities* a)
{
    ...
    for (i = 0; i < a->__sizeSupportedVersions; i++) {
        log_level_fmt(level + 1, "%d.%d\n", a->SupportedVersions[i].Major, a->SupportedVersions[i].Minor);
    }
}
...
void dump_tds__GetSystemDateAndTime(struct _tds__GetSystemDateAndTimeResponse* rep)
{
    ...
    if (NULL == systm->UTCDateTime) {
        ...
        log_fmt("UTCDateTime:       %04d-%02d-%02d %02d:%02d:%02d \n", systm->UTCDateTime->Date->Year,
                systm->UTCDateTime->Date->Month, systm->UTCDateTime->Date->Day, systm->UTCDateTime->Time->Hour,
                systm->UTCDateTime->Time->Minute, systm->UTCDateTime->Time->Second);
    }
    if (NULL == systm->LocalDateTime) {
        ...
        log_fmt("LocalDateTime:     %04d-%02d-%02d %02d:%02d:%02d \n", systm->LocalDateTime->Date->Year,
                systm->LocalDateTime->Date->Month, systm->LocalDateTime->Date->Day, systm->LocalDateTime->Time->Hour,
                systm->LocalDateTime->Time->Minute, systm->LocalDateTime->Time->Second);
    }
    log_func_out;
}
...
void dump_trt__GetServiceCapabilitiesResponse(struct _trt__GetServiceCapabilitiesResponse* rep)
{
    ...
    if (NULL != rep->Capabilities) {
        ...
        if (NULL != rep->Capabilities->ProfileCapabilities) {
            log_level_val(2, "MaximumNumberOfProfiles: ", log_int,
                          rep->Capabilities->ProfileCapabilities->MaximumNumberOfProfiles);
        }
        ...
        if (NULL != rep->Capabilities->StreamingCapabilities) {
            ...
            log_level_val(2, "RTP_USCORERTSP_USCORETCP: ", log_bool,
                          rep->Capabilities->StreamingCapabilities->RTP_USCORERTSP_USCORETCP);
            log_level_val(2, "NonAggregateControl: ", log_bool,
                          rep->Capabilities->StreamingCapabilities->NonAggregateControl);
...
void dump_trt__GetCompatibleVideoEncoderConfigurationsResponse(
    struct _trt__GetCompatibleVideoEncoderConfigurationsResponse* rep)
{
    int i;
    assert(NULL != rep);
...
void dump_trt__GetVideoEncoderConfigurationOptionsResponse(
