...
// A demo application, showing how to create and run a RTSP client (that can potentially receive multiple streams
// concurrently).
//
// NOTE: This code - although it builds a running application - is intended only to illustrate how to develop your own
// RTSP client application.  For a full-featured RTSP client application - with much more functionality, and many
// options - see "openRTSP": http://www.live555.com/openRTSP/
...
//     // This function call does not return, unless, at some point in time, "eventLoopWatchVariable" gets set to
//     something
//     // non-zero.

//     return 0;

//     // If you choose to continue the application past this point (i.e., if you comment out the "return 0;" statement
//     // above), and if you don't intend to do anything more with the "TaskScheduler" and "UsageEnvironment" objects,
//     then
//     // you can also reclaim the (small) memory used by these objects by uncommenting the following code:
//     /*
...
public:
    ...
    TyCallBackFuncFrame frameCallback;
    void*               args;
};
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString)
{
    do {
        ...
        if (scs.session == NULL) {
            env << *rtspClient
                << "Failed to create a MediaSession object from the SDP description: " << env.getResultMsg() << "\n";
            break;
        ...
        // 'subsessions', calling "MediaSubsession::initiate()", and then sending a RTSP "SETUP" command, on each one.
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString)
{
    do {
        ...
        // (This will prepare the data sink to receive data; the actual flow of data from the client won't start
        ...
        (( DummySink* )scs.subsession->sink)->frameCallback = (( ourRTSPClient* )rtspClient)->frameCallback;
        (( DummySink* )scs.subsession->sink)->args = (( ourRTSPClient* )rtspClient)->args;
...
void continueAfterPLAY(RTSPClient* rtspClient, int resultCode, char* resultString)
{
    ...
    do {
        ...
        // Set a timer to be handled at the end of the stream's expected duration (if the stream does not already signal
        // its end using a RTCP "BYE").  This is optional.  If, instead, you want to keep the stream active - e.g., so
        // you can later 'seek' back within it and do another RTSP "PLAY" - then you can omit this code. (Alternatively,
        // if you don't want to receive the entire stream, you could set this timer for some shorter value.)
        if (scs.duration > 0) {
...
void shutdownStream(RTSPClient* rtspClient, int exitCode)
{
    ...
    if (--rtspClientCount == 0) {
        ...
        // and replace it with "eventLoopWatchVariable = 1;", so that we leave the LIVE555 event loop, and continue
        // running "main()".)
        exit(exitCode);
    }
}
                                  ...
                                  unsigned /*durationInMicroseconds*/)
{
    ...
    //     envir() << fSubsession.mediumName() << "/" << fSubsession.codecName() << ":\tReceived " << frameSize << "
    //     bytes"; if (numTruncatedBytes > 0)
    //         envir() << " (with " << numTruncatedBytes << " bytes truncated)";
    //     char uSecsStr[6 + 1];  // used to output the 'microseconds' part of the presentation time
    //     sprintf(uSecsStr, "%06u", ( unsigned )presentationTime.tv_usec);
    //     envir() << ".\tPresentation time: " << ( int )presentationTime.tv_sec << "." << uSecsStr;
    //     if (fSubsession.rtpSource() != NULL && !fSubsession.rtpSource()->hasBeenSynchronizedUsingRTCP()) {
    //         envir() << "!";  // mark the debugging output to indicate that this presentation time is not
    //         RTCP-synchronized
    //     }
    ...
    if (frameCallback) {
        frameCallback(sps.sPropBytes, sps.sPropLength, fReceiveBuffer, frameSize, args);
    }
...
Boolean DummySink::continuePlaying()
{
    ...
    // arrives:
    fSource->getNextFrame(fReceiveBuffer, DUMMY_SINK_RECEIVE_BUFFER_SIZE, afterGettingFrame, this, onSourceClosure,
                          this);
    return True;
}
...
VSRtsp::VSRtsp() {}
VSRtsp::~VSRtsp() {}
int VSRtsp::open(char const* rtspURL, char const* sUser, char const* sPasswd, TyCallBackFuncFrame func, void* arg)
{
    TaskScheduler* scheduler = BasicTaskScheduler::createNew();
    ...
    // We need at least one "rtsp://" URL argument:
    // There are argc-1 URLs: argv[1] through argv[argc-1].  Open and start streaming each one:
    _rtspClient = ourRTSPClient::createNew(*_env, rtspURL, RTSP_CLIENT_VERBOSITY_LEVEL, NULL);
    if (_rtspClient == NULL) {
        // env << "Failed to create a RTSP client for URL \"" << rtspURL << "\": " << env->getResultMsg() << "\n";
        return -1;
    }
    _rtspClient->frameCallback = func;
    _rtspClient->args = arg;
    // Next, send a RTSP "DESCRIBE" command, to get a SDP description for the stream.
    // Note that this command - like all RTSP commands - is sent asynchronously; we do not block, waiting for a
    // response. Instead, the following function call returns immediately, and we handle the RTSP response later, from
    // within the event loop:
    Authenticator auth(sUser, sPasswd);
    _rtspClient->sendDescribeCommand(continueAfterDESCRIBE, &auth);
    return 0;
}

void VSRtsp::gotoNextframe()
{
    // _env->taskScheduler().doEventLoop(&eventLoopWatchVariable);
    BasicTaskScheduler0* env = (BasicTaskScheduler*)&_env->taskScheduler();
    env->SingleStep();
}
